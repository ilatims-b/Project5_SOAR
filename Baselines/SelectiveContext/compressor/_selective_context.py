"""
This is a modified version of the original Selective Context.

The original version is from https://github.com/THUDM/SelectiveContext

The main difference is that the original version are 
1. This version, the model type is compatible via transformers model name instead of hardcoding.
2. This version return boolean masked unit for deeper analysis purpose.
"""
...
class _SelectiveContext:
    ...
    def __init__(self, model_type = 'openai-community/gpt2', lang = 'en'):
    ...
    def _prepare_model(self):
        ...
        # if self.lang == 'zh':
        #     self.model = AutoModelForCausalLM.from_pretrained('uer/gpt2-chinese-cluecorpussmall')
        # else:
        #     self.model = AutoModelForCausalLM.from_pretrained('gpt2')
        # == New Implementation == 
        dtype = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
        self.tokenizer = AutoTokenizer.from_pretrained(self.model_type)
        self.model = AutoModelForCausalLM.from_pretrained(self.model_type, torch_dtype=dtype)
        ...
        self.get_self_information = self._get_self_info
        # == ==
    ...
    def _get_self_info(self, text: str) -> Tuple[List[str], List[float]]:
        # == Original Implementation ==
        # if self.lang == 'en':
        ...
        elif self.tokenizer.cls_token:
            text = self.tokenizer.cls_token + text
        elif self.tokenizer.eos_token:
            text = self.tokenizer.eos_token + text
        else:
            text = f"<|endoftext|>{text}"
        # == ==
        with torch.no_grad():
            ...
            # DEBUG:
            if encoding['input_ids'].shape[1] > self.max_token_length:
                print(f"[ERR] input_ids shape: {encoding['input_ids'].shape}")
                print(f"[ERR] text: {text}")
                return ["..."], [0.0]
        ...
        # Handle the case where input_ids is a single int (e.g., only one token in input)
        if isinstance(input_ids_list, int):
            input_ids_list = [input_ids_list]
        tokens = [self.tokenizer.decode(token_) for token_ in input_ids_list[1:]]
    ...
    def _get_self_info_via_curie(self, text: str) -> Tuple[List[str], List[float]]:
        # Legacy function from the original Selective Context
        num_retry = 3
    ...
    def self_info_mask(self, sents: List[str], self_info: List[float], mask_level):
        ...
        sents_after_mask = []
        masked_sents = []
        # This mask directly corresponds to the units being processed (sents, phrases, or tokens)
        unit_mask = []
        ...
        for sent, info in zip(sents, self_info):
            if info < self.ppl_threshold:
                ...
                unit_mask.append(False) # The unit was removed
            else:
                sents_after_mask.append(sent)
                unit_mask.append(True) # The unit was kept
        
        masked_context = " ".join(sents_after_mask) if mask_level == 'sent' else "".join(sents_after_mask)
        
        return masked_context, masked_sents, unit_mask
    ...
    def __call__(self, text: str, reduce_ratio: float = 0.35, reduce_level :str = 'phrase') -> List[str]:
        ...
        original_units = lexical_level[reduce_level].text
        context_after_mask, masked_sents, unit_mask = self.self_info_mask(original_units, lexical_level[reduce_level].self_info, reduce_level)
        
        return context_after_mask, masked_sents, unit_mask, original_units

def main(
    model_type = 'openai-community/gpt2', # you can choose from ['openai-community/gpt2' (new implementation), 'curie' (original impelmentation)]
    lang = 'en', # currenlty only support en and zh
    file_to_process: str = None,
    file_to_save: str = None,
):

    sc = _SelectiveContext(model_type=model_type, lang=lang)
...
if __name__ == "__main__":
    main(model_type='openai-community/gpt2', lang = 'zh')
