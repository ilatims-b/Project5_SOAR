...
def main():
    ...
    # === Modification: Load compressed dataset instead of original
    compressed_dataset_path = os.path.join(args.compressed_data_dir, "LongBench-v2-compressed")
    if os.path.exists(compressed_dataset_path):
        dataset = load_dataset(compressed_dataset_path, split='train')
    else:
        raise FileNotFoundError(f"Compressed dataset not found at {compressed_dataset_path}. Please run compress.py first.")
    # 
...
if __name__ == "__main__":
    ...
    # === Modification: add compressed data directory
    parser.add_argument("--compressed_data_dir", "-cd", type=str, default="compressed_data",
                       help="Directory containing the compressed dataset")
