...
def main(
    ...
    if "datapool" not in config and "dataset" in config:
        config["datapool"] = config["dataset"]
    if "reward_fn" not in config and "reward" in config:
        config["reward_fn"] = config["reward"]
    config["reward"] = config["reward_fn"]
    if config["datapool"]["name"] == "longbench2":
        train_ds = LongBench2Jsonl(config["datapool"]["args"]["train_path"])
        rf = config["reward"]["args"]               # <-- was config["reward"]
        reward_fn = MCReward(
            rf["gen_model_name"],
            penalty_lambda=rf["penalty_lambda"],
            max_new_tokens=rf["max_new_tokens"]
        )

        # ...pass train_ds to your dataloader, and reward_fn into the RL loop
