...
def wrap_onpolicy_alg(
    ...
    class OnPolicyAlgText(alg_class):
        def __init__(
            ...
            model_kwargs = dict(torch_dtype="auto", low_cpu_mem_usage=True)
            if gen_kwargs.get("trust_remote_code", True):
                model_kwargs["trust_remote_code"] = True
            apply_mp = gen_kwargs.get("apply_model_parallel", False)
            if has_accelerate and apply_mp:
                model_kwargs["device_map"] = "auto"
            self.gen_model = model_cls.from_pretrained(
                gen_kwargs['model_name']).eval()
        ...
        def compute_scst_rewards(self, env: VecEnv, obs_tensor: TensorDict, action_masks: np.ndarray):
            ...
            _, _, _, argmax_infos = zip(*results)  # len = n_envs

            sample_k = self.sample_k
            n_envs = env.num_envs
            assert len(argmax_infos) == n_envs, f"expected {n_envs} infos, got {len(argmax_infos)}"

            # Subsample each group of size sample_k
            kth_argmax_infos = argmax_infos[::sample_k]  # len = n_envs // sample_k
            ...
            rewards, _ = self._reward_fn(kth_argmax_infos, kth_gen_output, self.fixed_token_counts)

            # ---- Normalize to shape (n_envs,) robustly ----
            rewards = np.asarray(rewards).reshape(-1)  # handle scalar or list
            if rewards.size == 1:
                # scalar -> broadcast to all envs
                rewards = np.repeat(rewards, n_envs)
            elif rewards.size == n_envs // sample_k:
                # expand back to per-env by repeating each group
                rewards = np.repeat(rewards, sample_k, axis=0)
            elif rewards.size == n_envs:
                pass  # already good
            else:
                raise ValueError(
                    f"SCST rewards shape mismatch: got {rewards.shape}, "
                    f"expected 1, {n_envs // sample_k}, or {n_envs}"
                )

            # Make sure dtype is simple float
            rewards = rewards.astype(np.float32, copy=False)
            return rewards


        def _update_info_rewards(self, infos: List[Dict[str, Any]], rewards, rewards_dict: Dict[str, Any]):
            # broadcast scalars to per-env lists
            n = len(infos)
            if not isinstance(rewards, (list, tuple, np.ndarray)):
                rewards = [rewards] * n

            comp = rewards_dict.get('comp', 0.0)
            sim  = rewards_dict.get('sim', 0.0)
            if not isinstance(comp, (list, tuple, np.ndarray)):
                comp = [comp] * n
            if not isinstance(sim, (list, tuple, np.ndarray)):
                sim = [sim] * n

            for i, (info, reward) in enumerate(zip(infos, rewards)):
                info.setdefault("episode", {})
                info["episode"]["r"] = float(reward)
                info["episode"]["r_comp"] = float(comp[i])
                info["episode"]["r_sim"] = float(sim[i])
