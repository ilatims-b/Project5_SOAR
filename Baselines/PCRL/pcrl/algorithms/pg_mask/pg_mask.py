...
class MaskablePG(OnPolicyAlgorithm):
    ...
    def _get_eval_env(self, eval_env: Optional[GymEnv]) -> Optional[VecEnv]:
        """
        Wrap a plain Gymnasium env into a DummyVecEnv for evaluation if needed.
        If SB3 provides a base implementation in your version, you can remove this,
        but having it here ensures compatibility.
        """
        if eval_env is None:
            return None
        ...
        # Wrap single env -> Monitor -> DummyVecEnv
        def _make():
            try:
                return Monitor(eval_env)
            except Exception:
                # If Monitor fails (shouldn’t), just return the env
                return eval_env
    ...
    def train(self) -> None:
        ...
        bs = self.batch_size or (self.n_steps * self.n_envs)
        for rollout_data in self.rollout_buffer.get(bs):
