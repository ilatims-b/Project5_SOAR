...
class MaskableSCSTRolloutBuffer(MaskableDictRolloutBuffer):

    def compute_returns_and_advantage(self, use_scst, **kwargs):
        if use_scst:
            self._compute_scst_advantage(**kwargs)
        else:
            kwargs.pop('argmax_rewards', None)
            self._compute_returns_and_advantage(**kwargs)

    def _compute_scst_advantage(self, last_values: th.Tensor, dones: np.ndarray, argmax_rewards: np.ndarray):
        self.advantages = self.rewards - argmax_rewards

    def _compute_returns_and_advantage(self, last_values: th.Tensor, dones: np.ndarray) -> None:
